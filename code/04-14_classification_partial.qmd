...
```{r} 
#This is just a double check to see if is classifying correctly.
...
```{r}
#Another way to see the statistical distribution of fiber strength fo each of the classes.
...
best_accuracy <- rf_grid_result %>%
  select_best(metric = "accuracy") #accuracy is enough for us because we are using "quantiles" (balanced)
...
```{r final_spec}
final_spec <- rand_forest(trees = best_roc$trees,
                          mtry = best_roc$mtry) %>%
  # Specify the engine
  set_engine("ranger",
...
denominator <- 42 + 19 + 15 + 22 + 34 + 26 + 7 + 17 + 29

numerator/denominator  ##must match with the "collect_matrics" function on the cunck above
...
final_fit %>%
  ...
  roc_curve(strength_gtex_class, 
            ...
            .pred_Medium,
            .pred_High) %>%
ggplot(aes(x = 1 - specificity,  #Final exam
           y = sensitivity,         #Final exam
           color = .level)) +          #Final exam
  geom_line(size = 1.2) +
...
ggplot(aes(x = 1 - specificity, 
  ...
  #A perfect model would do this, as all the classes curves overlapping at 100%/1.00, since the model is properly classifying all the data. We can see through the "accuracy" formula on the chuncks above.
```
...
```{r}
final_spec %>%
  fit(strength_gtex_class ~ .,                        #Final exam.
         data = bake(weather_prep, 
                     weather_train)) %>%
    vi() %>%
  mutate(
...
#Almost the Same plot as we had on regression 
